...
<style>
    ...
    .gallery-preview img{
        width:110px;height:78px;object-fit:cover;border-radius:10px;border:1px solid var(--border);
    }
...
<div class="shell">
    <div class="page-header">
        <div>
            <div class="breadcrumbs"><a href="{{ route('admin.places.index') }}">Places</a> / <span>Edit</span></div>
    ...
    <div class="panel">
        <div class="panel-header">Update Place Details</div>
            ...
            <form action="{{ route('admin.places.update', $place) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-grid">
                    <div>
                        ...
                        <input type="text" name="name" id="name" class="field" value="{{ old('name', $place->name) }}" required>
                    </div>
                </div>
                ...
                <div class="form-grid split">
                    <div>
                        ...
                        @if ($place->image)
                            <img src="{{ asset('storage/' . $place->image) }}" alt="Place Image" width="120"
                                 style="border-radius:10px;border:1px solid var(--border);margin-bottom:8px"
                    ...
                    <div>
                        ...
                        <input type="text" name="location" id="location" class="field" value="{{ old('location', $place->location) }}">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" class="field">{{ old('description', $place->description) }}</textarea>
                </div>

                <div class="form-grid split">
                    <div>
                        <label for="weather">Weather</label>
                        <input type="text" name="weather" id="weather" class="field" value="{{ old('weather', $place->weather) }}">
                    </div>
                    <div>
                        <label for="travel_type">Travel Type</label>
                        <input type="text" name="travel_type" id="travel_type" class="field" value="{{ old('travel_type', $place->travel_type) }}">
                    </div>
                </div>
                ...
                <div class="form-grid split">
                    <div>
                        ...
                        <input type="text" name="season" id="season" class="field" value="{{ old('season', $place->season) }}">
                    </div>
                    <div class="form-grid split">
                        <div>
                            <label for="latitude">Latitude</label>
                            <input type="number" name="latitude" id="latitude" class="field" value="{{ old('latitude', $place->latitude) }}" step="0.00000001" min="-90" max="90">
                        </div>
                        <div>
                            <label for="longitude">Longitude</label>
                            <input type="number" name="longitude" id="longitude" class="field" value="{{ old('longitude', $place->longitude) }}" step="0.00000001" min="-180" max="180">
                        </div>
                    </div>
                </div>
                ...
                <div class="form-grid">
                    <div>
                        ...
                        @if ($place->gallery)
                            <div class="gallery-preview" style="margin-bottom:8px;">
                ...
                <div class="form-grid split">
                    <div>
                        ...
                        <input type="number" name="entry_fee" id="entry_fee" class="field" value="{{ old('entry_fee', $place->entry_fee) }}" step="0.01" min="0">
                    </div>
                    <div>
                        <label for="opening_hours">Opening Hours</label>
                        <input type="text" name="opening_hours" id="opening_hours" class="field" value="{{ old('opening_hours', $place->opening_hours) }}">
                    </div>
                </div>
                ...
                <div class="form-grid split">
                    <div>
                        ...
                        <input type="text" name="best_time_to_visit" id="best_time_to_visit" class="field" value="{{ old('best_time_to_visit', $place->best_time_to_visit) }}">
                    </div>
                    <div>
                        <label for="rating">Rating (0-5)</label>
                        <input type="number" name="rating" id="rating" class="field" value="{{ old('rating', $place->rating) }}" step="0.1" min="0" max="5">
                    </div>
                </div>
...
<script>
    ...
    // Province -> Districts (keeps old/selected district)
    ...
    const initialDistrict = @json(old('district', $place->district));
    ...
    function rebuildDistricts(){
        ...
        if (selected && provinceDistricts[selected]) {
            provinceDistricts[selected].forEach(d => {
                const opt=document.createElement('option');
                opt.value=d; opt.textContent=d;
                if (initialDistrict && initialDistrict === d) opt.selected = true;
