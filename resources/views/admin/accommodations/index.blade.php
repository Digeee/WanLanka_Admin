...
<style>
    ...
    .btn-ghost{
        ...
        border:1.5px dashed transparent;   /* width stable => no shake */
    }
    .btn-ghost:hover{
        border-style:solid;
    ...
    .alert-success{ color:#065f46; background:#ecfdf5; border:1px solid #a7f3d0; margin-bottom:16px; }
...
<div class="shell" id="accommodations-index-root">
    <div class="page-header">
        <div>
            <div class="breadcrumbs"><a href="{{ route('admin.dashboard') }}">Dashboard</a> / <span>Accommodations</span></div>
            <h1 class="title">Manage Accommodations</h1>
        </div>
        <div class="actions">
            <button class="btn btn-neutral" id="themeToggle" type="button" aria-label="Toggle theme">Theme</button>
            <a href="{{ route('admin.accommodations.create') }}" class="btn btn-primary">Add New Accommodation</a>
        </div>
    </div>
    ...
    <div class="panel">
        <div class="panel-header">All Accommodations</div>
                ...
                <table class="data">
                    ...
                    <tbody>
                        @foreach ($accommodations as $accommodation)
                            <tr>
                                <td style="font-weight:800;">{{ $accommodation->name }}</td>
                                <td>{{ $accommodation->province ?? 'N/A' }}</td>
                                <td>{{ $accommodation->district ?? 'N/A' }}</td>
                                <td style="font-weight:800;">${{ number_format($accommodation->price_per_night, 2) }}</td>
                                <td>{{ $accommodation->rating ? number_format($accommodation->rating, 1) : 'N/A' }}</td>
                                <td>
                                    <div class="actions">
                                        <a href="{{ route('admin.accommodations.show', $accommodation) }}" class="btn btn-ghost">View</a>
