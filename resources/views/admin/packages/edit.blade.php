    ...
    <div class="container">
        ...
        <form action="{{ route('admin.packages.update', $package) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="number" name="price" id="price" class="form-control" value="{{ old('price', $package->price) }}" step="0.01" min="0">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="date" name="starting_date" id="starting_date" class="form-control" value="{{ old('starting_date', $package->starting_date) }}">
            </div>
            <div class="form-group">
                <label for="expiry_date">Expiry Date</label>
                <input type="date" name="expiry_date" id="expiry_date" class="form-control" value="{{ old('expiry_date', $package->expiry_date) }}">
            </div>
            <div class="form-group">
                ...
                <select name="related_places[]" id="related_places" class="form-control" multiple required>
                    @foreach ($places as $place)
                        <option value="{{ $place->id }}" {{ in_array($place->id, old('related_places', $package->relatedPlaces->pluck('id')->toArray())) ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <select name="accommodations[]" id="accommodations" class="form-control" multiple required>
                    @foreach ($accommodations as $accommodation)
                        <option value="{{ $accommodation->id }}" {{ in_array($accommodation->id, old('accommodations', $package->accommodations->pluck('id')->toArray())) ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <select name="vehicle_type_id" id="vehicle_type_id" class="form-control" required>
                    <option value="" {{ old('vehicle_type_id', $package->vehicle_type_id) ? '' : 'selected' }}>Select Vehicle</option>
                    @foreach ($vehicles as $vehicle)
                        <option value="{{ $vehicle->id }}" {{ old('vehicle_type_id', $package->vehicle_type_id) == $vehicle->id ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <select name="package_type" id="package_type" class="form-control" required>
                    <option value="low_budget" {{ old('package_type', $package->package_type) == 'low_budget' ? 'selected' : '' }}>Low Budget</option>
                    <option value="high_budget" {{ old('package_type', $package->package_type) == 'high_budget' ? 'selected' : '' }}>High Budget</option>
